...
/*   Updated: 2026/09/14 09:39:48 by jkrishna         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	sift_down(t_heap *heap, char *scheduler)
{
	int			i;
	int			left;
	int			right;
	...
	while (1)
	{
		...
		smallest = i;
		if (left < heap->size
			&& is_smaller(heap->nodes[left], heap->nodes[smallest], scheduler))
			smallest = left;
		if (right < heap->size
			&& is_smaller(heap->nodes[right], heap->nodes[smallest], scheduler))
			smallest = right;
		if (smallest == i)
			break ;
		temp = heap->nodes[i];
